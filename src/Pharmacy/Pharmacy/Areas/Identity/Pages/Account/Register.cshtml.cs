    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                UserTypes type = UserTypes.Patient;
                if (type != Input.Type)
                    type = Input.Type;
                ...
                if (result.Succeeded)
                {
                    ...
                    switch (type)
                    {
                        case UserTypes.Doctor:
                            {
                                var item = await _userService.RegisterDoctorAsync(Input, user.Id);
                                user.Doctor = item;
                            }
                            break;
                        case UserTypes.Patient:
                            {
                                var item = await _userService.RegisterPatientAsync(Input, user.Id);
                                user.Patient = item;
                            }
                            break;
                        case UserTypes.Employee:
                            {
                                var item = await _userService.RegisterEmployeeAsync(Input, user.Id);
                                user.Employee = item;
                            }
                            break;
                    }
